...
class VideoRecurrentTestDataset(VideoTestDataset):
    ...
    def __init__(self, opt):
        ...
        self.folders = sorted(list(self.imgs_lq.keys()))
        ...
        for folder in ori_folders:
            ...
            for i in range((num_all - num_overlap) // (num_frame - num_overlap)):
                ...
                for j in range(num_frame):
                    clip_data_info['idx'].append(f'{j}/{num_frame}')
                border_l = [0] * num_frame
                for j in range(num_frame // 2):
                    border_l[j] = 1
                    border_l[num_frame - j - 1] = 1
...
@DATASET_REGISTRY.register()
class VideoRecurrentSplitClipsCropCenterTestDataset(VideoTestDataset):
    """Video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames. However, before
    that, the video frames are splited into clips, and crop center patch for
    fixing to definite size (gt_size).
    ...
    def __init__(self, opt):
        super(VideoRecurrentSplitClipsCropCenterTestDataset, self).__init__(opt)
    ...
    def __getitem__(self, index):
        ...
        h_lq, w_lq = imgs_lq.shape[-2:]
        assert h_lq >= lq_size and w_lq >= lq_size
        if h_lq != lq_size or w_lq != lq_size:
            h_ms = (h_lq - lq_size) // 2
            w_ms = (w_lq - lq_size) // 2
            imgs_lq = imgs_lq[:, :, h_ms:h_ms + lq_size, w_ms:w_ms + lq_size]
            h_ms = h_ms * scale
            w_ms = w_ms * scale
            imgs_gt = imgs_gt[:, :, h_ms:h_ms + gt_size, w_ms:w_ms + gt_size]
