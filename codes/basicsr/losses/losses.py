...
class GradientLoss(nn.Module):
    """L1 loss on images' gradient map.
    ...
    def __init__(self, loss_weight=1.0, reduction='mean'):
        ...
        self.sobel_filter_X = torch.from_numpy(sobel_filter_X).float()
        self.sobel_filter_Y = torch.from_numpy(sobel_filter_Y).float()
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        pred_grad_X = F.conv2d(pred, self.sobel_filter_X.type_as(pred), bias=None, stride=1, padding=1)
        pred_grad_Y = F.conv2d(pred, self.sobel_filter_Y.type_as(pred), bias=None, stride=1, padding=1)
        target_grad_X = F.conv2d(target, self.sobel_filter_X.type_as(pred), bias=None, stride=1, padding=1)
        target_grad_Y = F.conv2d(target, self.sobel_filter_Y.type_as(pred), bias=None, stride=1, padding=1)

        return self.loss_weight * (
                l1_loss(pred_grad_X, target_grad_X, weight, reduction=self.reduction) +
