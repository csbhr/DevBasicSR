...
from basicsr.utils.inference_util import inference_flipx4, inference_flipx8
...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def setup_optimizers(self):
        ...
        if flow_lr_mul == 1:
            ...
            if len(ignore_params) > 0:
                logger.info(f'Ignore {len(ignore_params)} params: [{ignore_params[0]} ...].')
            optim_params = [
    ...
    def test(self):
        ...
        self_ensemble = self.opt['val'].get('self_ensemble', None)
        self.center_frame_only = self.opt['val'].get('center_frame_only', False)

        if flip_seq:
            self.lq = torch.cat([self.lq, self.lq.flip(1)], dim=1)

        if self_ensemble is not None:
            with torch.no_grad():
                if self_ensemble == 'flipx4':
                    self.output = inference_flipx4(self.net_g, self.lq)
                elif self_ensemble == 'flipx8':
                    self.output = inference_flipx8(self.net_g, self.lq)
                else:
